...
public:
    // ── Tuning constants ────────────────────────────────────────────────
    static constexpr float CAUTION_DIST      = 2.5f;
    static constexpr float SLOW_DIST    = 3.5f; 
    float raw_front_dist_ = 999.f;

    
static constexpr float BRAKE_DIST   =1.2f;
    static constexpr float DANGER_DIST       = 1.5f;
    static constexpr float CLEAR_DIST        = 3.0f;
    static constexpr float LINEAR_SPEED_MPS  = 0.15f;  
    static constexpr float HARD_TURN_RAD_S   = 1.8f;
    static constexpr float  FRONT_FOV_DEG    = 30.0f;
static constexpr float  ESCAPE_GAP_WIDTH_DEG = 60.0f;  // gap must be this wide to drive through

    // ── Local minima / escape tuning ─────────────────────────────────────
    static constexpr double STUCK_TIMEOUT_S    = 4.0;   
    static constexpr float  STUCK_IMPROVE_M    = 0.25f; 
    static constexpr double ESCAPE_REVERSE_S   = 1.5;   
    static constexpr float  ESCAPE_REVERSE_MPS = -0.12f;
    static constexpr double ESCAPE_DRIVE_S = 8.0;
    static constexpr float  SIDE_FREE_THRESH    = 1.5f;  // side clearance = "I'm free"
    static constexpr float  ESCAPE_CLEAR_THRESH = 3.0f;
    ...
    float getRawFront() const { return raw_front_dist_; }

    AvoidState getState()   const { return state_;     }
    float      getFront()   const { return front_min_; }
    float getRear() const { return rear_min_; }

    void forceEscape(int attempt_count) {
    // Don't interrupt an ongoing escape — only force if stuck in AVOIDING
